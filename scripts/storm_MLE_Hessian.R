...
par_calcy <- foreach(co=1:cores, .combine= 'comb',
                      .init=list(list(), list(), list(), list(),
                       ...
                       #   MLtemp <- likfit(as.geodata(hurricane[,c(3,4,2)]), ini.cov.pars = ini.cov.likfit, lik.method = "ML",
                       ...
                       hess_time <- system.time({hur_opt <- optim(par = c(MLtemp$cov.pars, MLtemp$kappa), 
                                                                  fn = loglikhd.spatial, geodata=as.geodata(hurricane[,c(3,4,2)]),
...
if(exists("par_calcy")){save.image(file=paste0("~/NAM-Model-Validation/RData/RData",
