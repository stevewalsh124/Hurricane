...
if(subtractPWmean) {
  write.csv(svg.param.ests.error, "~/NAM-Model-Validation/csv/svg.param.ests.error_deg_subtractPWmean.csv")
} else {
  write.csv(svg.param.ests.error, "~/NAM-Model-Validation/csv/svg.param.ests.error_deg_noPWmean.csv")
}
...
if(makePWmean){
  ...
  writeRaster(PW_mean, "~/NAM-Model-Validation/error_rasters_summary/PW_mean", overwrite=T)
  ...
  writeRaster(S2, "~/NAM-Model-Validation/error_rasters_summary/S2", overwrite=T)
