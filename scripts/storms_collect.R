...
par(mfrow=c(2,2))
for (q in c(5,6,8,9)) {
  plot(all_storm_res[,q], old_fixnug[rownames(all_storm_res),q], main=colnames(all_storm_res)[q],
       xlab = NA, ylab=NA)
...
for (q in c(5,6,8,9)) {hist(all_storm_res[,q],main=colnames(all_storm_res)[q])}
for (q in c(5,6,8,9)) {hist(log(all_storm_res[,q]),main=paste("log",colnames(all_storm_res)[q]))}
